...
def generate_market_signals():
    ...
    try:
        ...
        return pd.DataFrame()
                    ...
                    st.warning(f"Advanced signal generation not available: {str(e)}")
                    st.info("Using built-in signal generation instead...")
                    
                    # Fallback to built-in signal generation
                    try:
                        # Use the built-in generate_market_signals function
                        signals = generate_market_signals()
                        if signals:
                            st.success(f"✅ Generated {len(signals)} trading signals using built-in analysis!")
